...
def generate_launch_description():
    ...
    delay_config = TimerAction(
        period=7.5,                             # Delay in seconds (global time)
    ...
    delay_node_1 = TimerAction(
        period=10.0,                            # Delay in seconds (global time)
    ...
    delay_node_2 = TimerAction(
        period=12.0,                            # Delay in seconds (global time)
    ...
    script_path = os.path.expanduser('~/sensors_ws/src/optris_drivers2/script/HALLEGADOELDIA.py') # mean_processor_filtered_nuria.py  or  mean_processor2_nuria.py
    mean_processor2_node = ExecuteProcess(
    ...
    rviz_config_file = os.path.expanduser('~/sensors_ws/src/realsense-ros/realsense2_camera/rviz2_config3.rviz') # path to configuration file
    rviz_node = Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
    ...
    delay_rviz = TimerAction(
        period=25.0,                            # Delay in seconds (global time)
        actions=[rviz_node]
    )  
