...
# Generate folder names based on current date
current_datetime = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
output_base_path = '/home/gerard/my_workspace_ros/src/Calibration/GraficosCalibracion/'
results_base_path = '/home/gerard/my_workspace_ros/src/Calibration/Results/'
...
for fname in thermal_images:
    ...
    index = os.path.basename(fname).split('_')[1].split('.')[0]
    ...
    if ret:
        ...
        # Save the image with chessboard corners
...
# Print the list of successful images
print(f"Images where a chessboard pattern was found: {successful_indices}")

# Combine all chessboard images into one large image with a white background
if chessboard_images:
    rows = len(chessboard_images) // 4 + 1
    cols = 4
    thumb_height, thumb_width = chessboard_images[0].shape[:2]
    ...
    combined_image_path = os.path.join(output_path, 'PUNTOSAjedrez.png')
...
# Check if the norm of the distortion coefficients is above a certain threshold
distortion_threshold = 1e-5  # Adjust this threshold as needed
if ret and np.linalg.norm(thermal_dist) > distortion_threshold:
    #------------------ Intrinsic ----------------------------------------%

    # Convert calibration parameters to JSON-compatible format
    # Guardar datos de calibración térmica en un archivo JSON
    ...
    k1, k2, p1, p2, k3 = thermal_dist.flatten()
...
# Save extrinsic parameters in a JSON file
extrinsic_data = {
...
# Save extrinsic parameters in a text file
...
#Images where a chessboard pattern was found: ['133', '138', '118', '12', '105', '136', '102', '134', '28', '101', '22', '29', '130', '5', '120', '2', '30', '14', '119', '129', '117', '20', '21', '135', '103', '38', '15', '96', '99', '97', '16', '128', '37', '137', '121', '19', '143', '100', '127', '17', '98', '131', '13', '18', '142', '116']