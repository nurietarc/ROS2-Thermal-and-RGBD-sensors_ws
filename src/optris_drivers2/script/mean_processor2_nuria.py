...
class MaskTemperatureCalculator(Node):
    ...
    def temperature_callback(self, thermal_msg):
        #self.get_logger().warn("Estoy en temperature_callback.")
        thermal_image = self.bridge.imgmsg_to_cv2(thermal_msg, desired_encoding='32FC1')
        self.temperature_image = (thermal_image - 1000) / 10.0
        #self.get_logger().info("Thermal image received")

        if self.rgb_image is not None:
            #self.get_logger().warn(f"self.temperature_image: {self.temperature_image} ")
            #self.get_logger().warn(f"self.H: {self.H} ")
            ...
            #self.get_logger().warn(f"self.warped_thermal: {self.warped_thermal} ")

            #self.get_logger().info("Warped image created")
        else:
    ...
    def compute_mask_temperature(self, mask, valid_pixels):
        #self.get_logger().warn("Estoy en compute_mask_temperature.")
        if self.warped_thermal is None:
            self.get_logger().warn("Warped thermal image is not available yet.")
            return 0.0

        mask_temperature_values = []
        #self.get_logger().warn(f"mask.shape[0]: {mask.shape[0]} ")
        #self.get_logger().warn(f"mask.shape[1]: {mask.shape[1]} ")
        #self.get_logger().warn(f"valid_pixels: {valid_pixels} ")
        for pixel in valid_pixels:
            x, y = pixel[1], pixel[0]
            #self.get_logger().warn(f"x: {x} ")
            #self.get_logger().warn(f"y: {y} ")
            if 0 <= x < mask.shape[1] and 0 <= y < mask.shape[0]:
                temperature = self.warped_thermal[y, x]
                if temperature > 0 and mask[y, x] > 0:
                    mask_temperature_values.append(temperature)

        #self.get_logger().warn(f"mask_temperature_values: {mask_temperature_values} ")
