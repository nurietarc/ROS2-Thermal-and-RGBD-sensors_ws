...
class MaskTemperatureCalculator(Node):
    def __init__(self):
        ...
        try:
            self.H = np.loadtxt(os.path.expanduser('~/sensors_ws/src/Calibration/Results/average_homography2.txt'))
        ...
        self.masked_image_pub = self.create_publisher(Image, '/masked_images', 10)
        ...
        self.masked_images = []
        self.desired_class_id = 'leaves'  # Set the desired class ID (e.g., 0 for "person")

        # Definition Twet and Tdry
        self.Twet = 22.0  # Appropriate value should be used (?)
        self.Tdry = 27.0  # Appropriate value should be used (?)
    ...
    def temperature_callback(self, thermal_msg):
        ...
        if self.rgb_image is not None:
            # self.get_logger().warn(f"self.rgb_image.shape[1]: {self.rgb_image.shape[1]} ")
            # self.get_logger().warn(f"self.rgb_image.shape[0]: {self.rgb_image.shape[0]} ")
            self.warped_thermal = cv2.warpPerspective(self.temperature_image, self.H, (self.rgb_image.shape[1], self.rgb_image.shape[0]))
            # self.get_logger().warn(f"self.thermal_warped_image.shape[1]: {self.warped_thermal.shape[1]} ")
            # self.get_logger().warn(f"self.thermal_warped_image.shape[0]: {self.warped_thermal.shape[0]} ")
            ...
            for i, mask_image_msg in enumerate(yolo_result_msg.masks):
                ...
                if self.warped_thermal is not None:
                    ...
                    if mask_temperature != 0.0:
                        # Compute CWSI using mask's mean temperature
                        cwsi = (mask_temperature - self.Twet) / (self.Tdry - self.Twet)

                        # Publish mask's mean temperature
                        mask_temperature_msg = Float32()
                        mask_temperature_msg.data = float(mask_temperature)
                        self.mask_temperature_pub.publish(mask_temperature_msg)
    ...
    def draw_text_on_mask(self, mask, temperature, cwsi):
        ...
        cv2.putText(masked_image_with_text, f'CWSI: {cwsi:.2f}', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
#         self.rgb_sub = self.create_subscription(Image, '/camera/camera/color/image_raw', self.rgb_callback, 10)
#         self.temperature_sub = self.create_subscription(Image, '/thermal_image', self.temperature_callback, 10)
#         self.mask_sub = self.create_subscription(YoloResult, '/yolo_result', self.mask_callback, 10)
...
#         self.rgb_image = None
#         self.warped_thermal = None
#         self.desired_class_id = 'person'

#         self.Twet = 22.0
#         self.Tdry = 27.0

#         self.is_processing = False  # Variable para controlar si se está procesando una máscara
...
#             self.warped_thermal = cv2.warpPerspective(self.temperature_image, self.H, (self.rgb_image.shape[1], self.rgb_image.shape[0]))
#             warped_thermal_msg = self.bridge.cv2_to_imgmsg(self.warped_thermal, encoding='32FC1')
#             self.warped_image_pub.publish(warped_thermal_msg)
#             self.get_logger().info("Published warped thermal image")
#         else:
#             self.get_logger().warn("RGB image is not available yet.")
...
#         self.get_logger().info("Starting mask processing...")

#         Procesar la máscara solo si no se está procesando ninguna
#         if self.is_processing:
#             self.get_logger().info("Already processing, discarding new mask.")
#             return

#         if yolo_result_msg.masks and yolo_result_msg.detections.detections:
#             self.is_processing = True  # Iniciar el procesamiento
...
#                         self.get_logger().warn("Warped thermal image is not available yet.")
#             finally:
#                 Asegurarse de que la variable se restablezca incluso si ocurre un error
#                 self.is_processing = False  

#                 Medir el tiempo final y loggear el tiempo transcurrido
#                 end_time = time.time()
#                 elapsed_time = end_time - start_time
#                 self.get_logger().info(f"Mask processed in {elapsed_time:.2f} seconds")
#         else:
#             self.get_logger().warn('Received empty masks or detections list.')
...
#     finally:
#         if rclpy.ok():
#             mask_temp_calculator.get_logger().info("Shutting down node.")
#             rclpy.shutdown()
